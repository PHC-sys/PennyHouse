...
#### ★ 전략 발견 — Funding Swap-Spread Carry (2026-06-16)
```
게스트-xmlj 발견 + 부장님 확인. 스펙: docs/specs/FundingSwapSpreadCarry_spec.md
- 유사자산(ETF↔구성종목, 인덱스↔대형주)의 펀딩 괴리를 perp-only 롱/숏으로 수확.
  펀딩=베이시스="크립토 채권 금리". 캐리수취⇄베이시스수렴(가격손익)은 동전 양면.
  방향 무관(스프레드 부호 따라 스위칭)이 강점.
- 유동성 triage(실데이터): 페어1 Long SKHX+SMSN / Short DRAM = 세 레그 전부 유동
  (OI $20~116M) → 1순위 검증. KR200/USOIL/ENERGY는 죽은 시장(=발견자 '헷지 안 됨' 관측).
- 합격 기준(백테스트): 방향무관 캐리가 휩쏘·확대구간(경로생존)·비용 다 이기고 net+.
- ★ 다음 작업 1순위 후보 = 이 전략 백테스트 하니스(backtest/FSSC/). 제품 핵심 논제 검증.
  (리플레이 4차보다 우선 권고 — 펀드의 '알파 존재'를 먼저 증명)
```
