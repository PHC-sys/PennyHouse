...
✅ 3.5차: 운영 보강 (2026-06-15)
        ...
        ✅ 시가평가 mid→markPx (HIP-3 손익 정확. live.get_marks/ensure_ctx)
✅ 4차(선): NAV 재구성 엔진 — ★ 메모리 누적 폐기, 이벤트 소싱 (2026-06-15)
        ✅ 진실의 원천 = 불변 이벤트 원장(store.fund_events): 비중변화=rebalance, 출금=redeem
        ✅ NAV/equity/손익 = 원장+정규캔들가격을 replay한 순수함수 (api/nav.py reconstruct)
        ✅ 결정론(같은 입력=같은 출력)·재시작 불변(메모리0에서 복원, 리셋 없음)·다중인스턴스 안전
        ✅ 라이브 팁만 현재 markPx로 실시간 보정. _runtime은 표시용 캐시(진실 아님)
        ✅ 실펀드(5차)는 원장 대신 HL clearinghouseState 읽어 같은 엔진 재사용
        ※ 구 스냅샷 누적(fund_runtime)은 백필 폴백으로만 잔존. nav_history 미사용(직접 산출)
...
→ 다음 할 일(플랫폼): 시뮬레이터 UI(역사책=과거 백테스트 탭 대체, engine/simulate.py 준비됨)
   or 작은 보강(펀딩 NAV 폴드 / 분류·검색 별칭). ※ FSSC 전략은 폐기됨(아래 참조).
```

#### ★ 전략 검증 결과 — Funding Swap-Spread Carry: 엣지 없음, 폐기 (2026-06-16~22)
```
게스트-xmlj 발견 가설을 철저히 검증 → 배포 가능한 시스템 엣지 없음(폐기).
스펙/결론 docs/specs/FundingSwapSpreadCarry_spec.md · 스크립트/데이터 backtest/FSSC/
- 검증 요약(전부 실패): KR200 78%=유동성 아티팩트(유동 페어선 31%) / 펀딩카리·평균회귀=짐
  / 모멘텀=음의 중앙값·팻테일 / 공적분 stat-arb=인샘플 78%였으나 OOS 3split 전부 폭망(-860%).
- 결론: 펀딩=mark−oracle 베이시스인데 시장이 효율적으로 닫아둠(베이시스 평균 ~0.07%). 공짜 신호 없음.
- ★ 단 '동일자산 교차dex 차익'(GOLD/SILVER xyz vs cash, 실측 +0.097%net 가격갭 + 펀딩 17%/yr)은
  구조적으로 유효 = 신호 게임이 아니라 '운영 게임'. → 차익 코크핏 백로그(roadmap 6차 뒤 참조).
- ⚠️ 새 세션 주의: 이 신호전략 재탐색 금지(다 해봤고 OOS 사망). 코크핏은 기존 플랫폼보다 후순위.
```
...
[페이퍼(지금)]  지갑 없음 → 서버가 계산. 단, equity/NAV/손익을 메모리에 '누적'하지
               않음(2026-06-15 전환). 진실의 원천 = 불변 이벤트 원장(fund_events).
               NAV는 원장+정규가격을 매번 replay한 순수함수(nav.reconstruct)라
               결정론적·재시작 불변. → "재시작하면 사라진다" 문제 구조적으로 소멸.
               (전엔 메모리 누적이라 mid/mark·리셋·절벽 등 같은 뿌리의 버그 다발)
